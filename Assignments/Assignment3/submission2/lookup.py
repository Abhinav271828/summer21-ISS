...
def matches(fields, row):
    ...
    """
    if (list(fields.values()) == row[1:5]):     # Compare fields with row values
        return True
    else:
        return False
...
def getresults():
    ...
    for row in reader:
        match = [person['results'] for person in rules if matches(person['fields'],row) and person['status'] == 'Active']
        if (match == []):
            res = 'No Access'
        else:
            res = match[0]
